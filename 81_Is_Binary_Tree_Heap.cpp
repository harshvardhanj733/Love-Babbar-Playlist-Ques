...
class Solution {
  private:
    ...
    bool isCBT(struct Node* tree, int i, int c){
        if(tree == NULL){
            return true;
        }
        if(i >= c){
        ...
        return isCBT(tree->left, 2*i + 1, c) && isCBT(tree->right, 2*i + 2, c);
    }
    ...
    bool isMaxHeap(struct Node* tree){
        ...
        if(tree->left && tree->left->data > largest){
            largest = tree->left->data;
        }
        if(tree->right && tree->right->data > largest){
